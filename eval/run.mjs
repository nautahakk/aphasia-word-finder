// Re-run the blind test set through the app's own code and write eval/RESULTS.md.
// Usage: TYPESAFE_API_KEY=... npm run eval        (~420 API calls, a few cents)
//
// cases.json: 105 descriptions written by a separate AI agent to imitate aphasic
// speech as speech-to-text would transcribe it. The agent never saw the method.
// They are SIMULATED - not real people with aphasia.
...
for (const c of cases) {
  const full = await findWord({ said: c.said, personal: DEMO_WORDS, apiKey: KEY });
  const half = await findWord({ said: firstHalf(c.said), personal: DEMO_WORDS, apiKey: KEY });
  rows.push({
...
const lines = [
  `# Evaluation results`,
  ``,
  `Run: ${new Date().toISOString().slice(0, 10)} · model: jev-latest · ${rows.length} cases (${scored.length} answerable, ${unanswerable.length} unanswerable)`,
  ``,
